...
class ASPP_module_rate0(nn.Module):
    ...
    def forward(self, x):
        x = self.atrous_convolution(x)
        if self.training and x.size(2) <= 1 and x.size(3) <= 1:
            self.bn.eval()
            with torch.no_grad():
                x = self.bn(x)
            self.bn.train()
...
class Block2(nn.Module):
    def __init__(self, inplanes, planes, reps, stride=1, dilation=1, start_with_relu=True, grow_first=True,
        ...
        if stride != 1:
            self.block2_lastconv = nn.Sequential(
                *[self.relu, SeparableConv2d_same(planes, planes, 3, stride=stride, dilation=dilation)])
        if is_last:
    ...
    def forward(self, inp):
        ...
        low_middle = x.clone()
        x1 = x
        x1 = self.block2_lastconv(x1) if hasattr(self, 'block2_lastconv') else x1
        if self.skip is not None:
            skip = self.skip(inp)
            skip = self.skipbn(skip)
        else:
...
class Xception(nn.Module):
    def __init__(self, inplanes=1, os=16, pretrained=False):
        ...
        self.conv3 = SeparableConv2d_aspp(512, 768, 3, stride=1, dilation=exit_block_rates[1],
                                          padding=exit_block_rates[1])
        self.conv4 = SeparableConv2d_aspp(768, 768, 3, stride=1, dilation=exit_block_rates[1],
                                          padding=exit_block_rates[1])
        self.conv5 = SeparableConv2d_aspp(768, 1024, 3, stride=1, dilation=exit_block_rates[1],
                                          padding=exit_block_rates[1])
...
class DeepLabv3_plus_gcn_skipconnection_2d(nn.Module):
    def __init__(self, nInputChannels=1, n_classes=17, os=16, pretrained=False, _print=True, final_sigmoid=False,
        ...
        self.decoder1 = nn.Sequential(Decoder_module(152, 128), Decoder_module(128, 128))
        self.decoder2 = nn.Sequential(Decoder_module(192, 256), Decoder_module(256, 256))
        if self.gcn_mode == 0:
            ...
            self.graph_2_fea = gcn.Graph_to_Featuremaps_savemem(input_channels=128, output_channels=128,
            ...
            self.graph_2_fea = gcn.My_Graph_to_Featuremaps(hidden_layers=hidden_layers, output_channels=128,
                                                           dimension=2)
        self.skip_conv = nn.Sequential(nn.Conv2d(128, 128, kernel_size=1), nn.ReLU(True))
        self.semantic = nn.Conv2d(256, n_classes, kernel_size=1, stride=1)
        self.final_activation = nn.Softmax(dim=1) if not final_sigmoid else nn.Sigmoid()
    ...
    def forward(self, img, unary=None, return_feat=False, **kwargs):
        input = img
        if unary is not None and self.in_channels > 1:
            pre_seg = F.interpolate(unary, size=input.shape[2:], mode='bilinear', align_corners=True)
            input = torch.cat((input[:, :1, ...], pre_seg), dim=1)
            assert input.size(1) == self.in_channels, f"Expected {self.in_channels} channels, got {input.size(1)}"
        adj = self.get_graph()
        ...
        if x5.size(2) <= 1 and x5.size(3) <= 1:
            x5 = F.interpolate(x5, size=(x.size(2), x.size(3)), mode='bilinear', align_corners=True)
        x = torch.cat((x1, x2, x3, x4, x5), dim=1)
        x_before_bn = self.concat_projection_conv1(x)
        x = self.concat_projection_bn1(x_before_bn)
        x = self.relu(x)
        ...
        if x.size(2) <= 1 or x.size(3) <= 1:
            ...
            if self.gcn_mode == 1:
                ...
                if spine_graph.size(2) != x.size(2) or spine_graph.size(3) != x.size(3):
                    spine_graph = F.interpolate(spine_graph, size=x.size()[2:], mode='bilinear', align_corners=True)
                fea_logit = None
            else:
                raise NotImplementedError("Only gcn_mode=1 is supported in this context")
    ...
    def load_state_dict_new(self, state_dict):
        ...
        for name, param in state_dict.items():
            ...
            try:
                ...
                print(
                    f'While copying the parameter named {name}, whose dimensions in the model are {own_state[name].size()} and whose dimensions in the checkpoint are {param.size()}, ...')
                continue
...
if __name__ == "__main__":
    ...
    unary = torch.randn(2, 20, 256, 256).to(device)
    model = DeepLabv3_plus_gcn_skipconnection_2d(nInputChannels=18, n_classes=17, os=16, pretrained=False, _print=True,
                                                 gcn_mode=1, device=device)
    model = model.to(device)
    model.eval()
    with torch.no_grad():
        output, _ = model.forward(image, unary=unary)
    print(output.size())